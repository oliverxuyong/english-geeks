...
- **Blanks**: same length as `words[]`; each slot is either the visible token or a blank string (`____`, `_____`, or partial letters). Generated by `blank_out_designated_words.py` (spaCy levels 2 / 4 / 5 → beginner / intermediate / advanced) when spaCy is installed; else `blankRules.mjs`.
